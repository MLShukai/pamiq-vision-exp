...
2. **ブランチ作成**: `<種別>/<日付>/<内容>` で作業ブランチを切る
3. **実装**: コードを書く
4. **検証**: `make format && make type && make test` を実行し、全てパスすることを確認する
5. **コミット**: 検証が通ったらコミットする。細かい単位でコミットし、1コミットに複数の関心事を混ぜない
6. **繰り返し**: 3-5 を機能単位で繰り返す

### ブランチとマージ

- 機能ごとに作業ブランチを作成する
- 作業完了後、`main` にマージする（`git merge --no-ff`）
- マージ後、作業ブランチを削除する
